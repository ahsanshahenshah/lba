...
grayscale = 0
input_shape = (96,96)
def _parse_function_sup(filename,label):
...
def _parse_function_unsup(filename):
    image_string = tf.read_file(filename)
    image_decoded = tf.cast(tf.image.decode_jpeg(image_string, channels=3), tf.float32)
    image_resized = tf.image.resize_images(image_decoded, (input_shape[0], input_shape[1]))
    if grayscale:
...
def create_input(input_images, input_labels, batch_size):
    ...
    """
    print ('input_images array rank',len(np.shape(input_images)))
    if len(np.shape(input_images))==1:
        if input_labels is None:
            train_dataset = tf.data.Dataset.from_tensor_slices(input_images)
            buffersize = min(len(input_images), 10000)
            train_dataset = train_dataset.shuffle(buffer_size=buffersize, seed=1)
            train_dataset = train_dataset.map(_parse_function_unsup, num_parallel_calls=5).repeat().batch(batch_size)
        else:
            train_dataset = tf.data.Dataset.from_tensor_slices((input_images, input_labels))
            buffersize = min(len(input_images), 10000)
            train_dataset = train_dataset.shuffle(buffer_size=buffersize, seed=1)
            train_dataset = train_dataset.map(_parse_function_sup, num_parallel_calls=1).repeat().batch(batch_size)
...
def create_per_class_inputs(image_by_class, n_per_class, class_labels=None):
    ...
    """
    print ('images per class',np.shape(image_by_class),n_per_class)
...
def sample_by_label(images, labels, n_per_label, num_labels, seed=None):
    ...
    print(n_per_label, num_labels, labels==1)
    res = []
